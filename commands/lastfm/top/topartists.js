    ...
    db.get(
      ...
      async (err, row) => {
        ...
        try {
          ...
          const artists = top.data.topartists.artist;
          if (!artists || artists.length === 0) {
            return respond({
              response_type: 'ephemeral',
              text: `No top artists found for *${username}*.`,
            });
          }
          ...
          artists.forEach((artist, index) => {
            const imageUrl = artist.image?.find(
          ...
          await respond({
            response_type: 'ephemeral',
            text: '⚠️ Could not fetch top artists.',
          });
        }
      }
    );
